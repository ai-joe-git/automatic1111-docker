...
ENV DEFAULT_COMMANDLINE_ARGS="--data-dir ${DATA_DIR} --ckpt-dir ${CKPT_DIR} --vae-dir ${VAE_DIR} --lora-dir ${LORA_DIR} --embeddings-dir ${EMBEDDINGS_DIR} --no-hashing --do-not-download-clip --precision full --no-half --skip-torch-cuda-test --use-cpu all --enable-insecure-extension-access"
